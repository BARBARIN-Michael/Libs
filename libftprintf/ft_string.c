...
/*   Updated: 2015/02/16 20:54:31 by mbarbari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*handle_wstr(va_list ap, t_opt *opt)
{
	...
	while (*c)
	{
		...
		try == 0 ? str[0] = '\0' : 0;
		try = 1;
		if (!(str = ft_strjoin(str, ft_wtoa(*c))))
			ft_putendl_c("ERROR X102.2 : ERROR memory", "red");
		c++;
	}
	opt->precision = 0;
...
char	*handle_char(va_list ap, t_opt *opt)
{
	...
	if (!(c = malloc(2)))
	{
		ft_putendl_c("ERROR X102.1 : ERROR memory", "red");
		return (NULL);
	}
